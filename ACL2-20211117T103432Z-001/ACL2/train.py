...
train_display_images_b = torch.stack([torch.tensor(train_loader_b.dataset[i]) for i in range(display_size)]).cuda()
# test_display_images_a = torch.stack([test_loader_a.dataset[i] for i in range(display_size)]).cuda()
# test_display_images_b = torch.stack([test_loader_b.dataset[i] for i in range(display_size)]).cuda()
print(len(train_loader_a.dataset))
...
now = time.strftime("%c")
log_csv = os.path.join(checkpoint_directory, 'loss_log_(%s).csv'%now)
...
while True:
    for it, (images_a, images_b) in enumerate(zip(train_loader_a, train_loader_b)):
        ...
        if (iterations + 1) % config['log_iter'] == 0:
            ...
            write_loss(iterations, trainer, log_csv,flag)
            flag = False
