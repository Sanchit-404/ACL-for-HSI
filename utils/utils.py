...
# def get_custom_data_loaders(conf):
...
def get_all_data_loaders(conf):
    ...
    if 'data_root' in conf:
        train_loader_a = get_data_loader_folder(conf['path_A'], conf, batch_size, True,
                                              new_size_a, height, width, num_workers, True, datakind,data_type='RGB')
...
#                                              new_size_a, new_size_a, new_size_a, num_workers, True, datakind)
        train_loader_b = get_data_loader_folder(conf['path_B'], conf, batch_size, True,
                                              new_size_b, height, width, num_workers, True, datakind,data_type='HSI')
...
def get_data_loader_folder(input_folder, conf, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True, datakind='',data_type='RGB'):
...
#     dataset = ImageFolder(input_folder, transform=transform)
    ...
    dataset = CustomDataset(input_folder,conf,data_type,transform)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=train, drop_last=True, num_workers=num_workers)
    return loader
...
def tensor2hsi(input_image, imtype=np.float32):
    ...
    if not isinstance(input_image, np.ndarray):
        if isinstance(input_image, torch.Tensor):
            ...
            return input_image
        # print(input_image.data[0].shape)
...
def __write_images(image_outputs,dict, display_image_num, image_directory,postfix):
    i = 0 
    for images in image_outputs:
        ...
        for image in images:
            ...
            im = tensor2hsi(image)
            # print(im.shape)
            save_path = os.path.join(image_directory,dict[i]+postfix+'.hdf5')
        ...
        # print(images.shape)
        # if images.shape[1]==3:
        #     image_outputs = images.expand(-1, 3, -1, -1)
        # else :
        #     images_outputs = images.expand(-1, 31, -1, -1)
...
def write_2images(image_outputs, display_image_num, image_directory, postfix):
    n = len(image_outputs)
    dict = {1:'x_A',2:'x_A_fake',3:'x_B_fake', 4:'x_A2_fake', 5:'x_A_recon', 6:'x_B', 7:'x_B_recon'}
    __write_images(image_outputs[0:n], dict, display_image_num, image_directory,postfix)        

# def save_images(image_outputs, display_image_num, image_directory, file_name, hsi=False):
#     x_A, x_A_fake, x_B_fake, x_A2_fake, x_A_recon, x_B, x_B_recon = image_outputs
#     img_array = [x_A, x_A_fake, x_B_fake, x_A2_fake, x_A_recon, x_B, x_B_recon]
#     for img in img_array:
#         if img.shape[1] == 3:
#             img = img.expand(-1, 3, -1, -1)
#         else:
#             img = img.expand(-1, 31, -1, -1)
...
def write_loss(iterations, trainer, log_csv,flag):
    ...
    if flag:
        for m in members:
            header += ', %s' %(m)
        with open(log_csv, "w") as log_file:
            log_file.write(header)
   
    message = '\n'+str(iterations)
    message_to_print = 'Iter ' + str(iterations)
    for m in members:
        message += ', %f' %((getattr(trainer,m)).item())
        message_to_print += ', %s : %0.3f' %(m[5:],(getattr(trainer,m)).item())
    with open(log_csv, "a") as log_file:
        print(message_to_print)
        log_file.write(message)

            # print(iterations + 1)
            # log_file.write(m, getattr(trainer, m), iterations + 1)
