...
"""
from networks import AdaINGenEnc, AdaINGenDec, MsImageDis, VAEGen
from utils.utils import weights_init, get_model_list, vgg_preprocess, load_vgg16, get_scheduler
...
class aclgan_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        ...
        # self.gen_AB = AdaINGen(hyperparameters['input_dim_a'], hyperparameters['gen'], output_dim=31)  # 3-31 auto-encoder for domain A
        # self.gen_BA = AdaINGen(hyperparameters['input_dim_a'], hyperparameters['gen'], output_dim=3)  # 3-3 auto-encoder for domain B
        self.gen_A_enc = AdaINGenEnc(hyperparameters['input_dim_a'], hyperparameters['gen'])  # 3-enc encoder for domain A (encode domain A)
        self.gen_B_dec = AdaINGenDec(hyperparameters['input_dim_a'], hyperparameters['gen'], output_dim=31) #enc-31 to domain B (decode to domain B)
        
        
        self.gen_B_enc = AdaINGenEnc(hyperparameters['input_dim_b'], hyperparameters['gen'])  # 31-enc encoder for domain B (encode domain B)
        self.gen_A_dec = AdaINGenDec(hyperparameters['input_dim_b'], hyperparameters['gen'], output_dim=3) #enc-3 to domain A (decode to domain A)
        
        
        ## self.gen_AB_hsi = AdaINGen(hyperparameters['input_dim_b'], hyperparameters['gen'], output_dim=31) # 31-31
        ## self.gen_BA_hsi = AdaINGen(hyperparameters['input_dim_b'], hyperparameters['gen'], output_dim=3) # 31-3
        self.dis_A = MsImageDis(hyperparameters['input_dim_a'], hyperparameters['dis'])  # discriminator for domain A
        self.dis_B = MsImageDis(hyperparameters['input_dim_b'], hyperparameters['dis'])  # discriminator for domain B
        # self.dis_B_1 = MsImageDis(hyperparameters['input_dim_b'], hyperparameters['dis'])
        self.dis_2 = MsImageDis(2*hyperparameters['input_dim_a'], hyperparameters['dis'])  # discriminator 2
        # self.dis_2_hsi = MsImageDis(hyperparameters['input_dim_b'], hyperparameters['dis'])  # discriminator 2 for domain B
        ...
        gen_params = list(self.gen_A_enc.parameters()) + list(self.gen_B_dec.parameters()) + list(self.gen_B_enc.parameters()) + list(self.gen_A_dec.parameters())
    ...
    def forward(self, x_a, x_b):
        ...
        c_4, s_4 = self.gen_AB_hsi.encode(x_b)
        # decode
        ...
        #encode 2
        c_3, _ = self.gen_BA_hsi.encode(self.x_B_fake)
    ...
    def channel_alteration(self,x,out_channels):
        # print(x.shape)
        output = []
        if out_channels == 31:
            for item in x:
                ...
                out_img = np.concatenate((B_img, G_img, R_img), axis=2)
                # print(out_img.shape)
                out_img = np.einsum('abc -> cab',out_img)
                output.append(torch.unsqueeze(torch.from_numpy(out_img),0))
                ...
                for item in x:
                    image = item.cpu().float().detach().numpy()
                    # image = x.cpu().float().detach().numpy()
                    r = image[R_,:,:]
                    ...
                    # out_img = self.transform_rgb(image)
                    output.append(torch.unsqueeze(torch.from_numpy(rgb).type(torch.FloatTensor),0))
        return torch.cat(output,0).cuda()
    ...
    def gen_update(self, x_a, x_b,  hyperparameters):
        ...
        z_3 = Variable(torch.randn(x_a.size(0), self.style_dim, 1, 1).cuda())
        # encode
        c_1, _  = self.gen_A_enc.encode(x_a)
        c_2, s_2 = self.gen_B_enc.encode(self.channel_alteration(x_a,31))
        c_4, s_4 = self.gen_A_enc.encode(self.channel_alteration(x_b,3))
...
#            x_B_recon = self.focus_translation(x_B_recon, x_b, x_B_recon_focus)
            ...
            x_B_fake = self.gen_B_dec.decode(c_1, z_1)
            x_A_fake = self.gen_A_dec.decode(c_2, self.alpha * z_2)
            # recon
            x_A_recon = self.gen_A_dec.decode(c_2, s_2)
            x_B_recon = self.gen_B_dec.decode(c_4, s_4)

        #encode 2
        c_3, _ = self.gen_B_enc.encode(x_B_fake)
        if focus_lambda > 0:
            x_A2_fake, x_A2_focus = self.gen_BA.decode(c_3, z_3).split(3, 1)
            x_A2_fake = self.focus_translation(x_A2_fake, x_B_fake, x_A2_focus)
        else:
            x_A2_fake = self.gen_A_dec.decode(c_3, z_3)
...
#                x_A_recon.append(self.focus_translation(img, x_a[i].unsqueeze(0), mask))
                ...
                c_1, s_1 = self.gen_B_enc.encode(self.channel_alteration(x_a[i].unsqueeze(0),31))
                x_A_fake.append(self.gen_A_dec.decode(c_1, z_1[i].unsqueeze(0)))
                x_A_recon.append(self.gen_A_dec.decode(c_1, s_1))
                

                c_2, _ = self.gen_A_enc.encode(x_a[i].unsqueeze(0))
                x_B1 = self.gen_B_dec.decode(c_2, z_2[i].unsqueeze(0))
                x_B_fake.append(x_B1)

                # c_3, _ = self.gen_BA_hsi.encode(x_B1)
                c_3, _ = self.gen_B_enc.encode(x_B1)
                x_A2_fake.append(self.gen_A_dec.decode(c_3, z_3[i].unsqueeze(0)))

                # c_4, s_4 = self.gen_AB_hsi.encode(x_b)
                c_4, s_4 = self.gen_A_enc.encode(self.channel_alteration(x_b,3))
                x_B_recon.append(self.gen_B_dec.decode(c_4, s_4))
    ...
    def dis_update(self, x_a, x_b, hyperparameters):
        ...
        # decode
        if focus_lambda > 0:
            x_B_fake, x_B_focus = self.gen_AB.decode(c_1, z_1).split(31, 1)
            x_A_fake, x_A_focus = self.gen_BA.decode(c_2, self.alpha * z_2).split(3, 1)
            x_B_fake = self.focus_translation(x_B_fake, x_a, x_B_focus)
            x_A_fake = self.focus_translation(x_A_fake, x_a, x_A_focus)
        else:
    ...
    def resume(self, checkpoint_dir, hyperparameters):
        ...
        self.gen_A_enc.load_state_dict(state_dict['A_enc'])
        self.gen_B_enc.load_state_dict(state_dict['B_enc'])
        self.gen_A_dec.load_state_dict(state_dict['A_dec'])
        self.gen_B_dec.load_state_dict(state_dict['B_dec'])
    ...
    def save(self, snapshot_dir, iterations):
        ...
        torch.save({'A_enc': self.gen_A_enc.state_dict(), 'B_enc': self.gen_B_enc.state_dict(),'A_dec':self.gen_A_dec.state_dict(), 'B_dec': self.gen_B_dec.state_dict()}, gen_name)
